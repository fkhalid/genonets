...
from genonets.genonets_utils import Utils
...
class PeakAnalyzer:
    ...
    def __init__(self, network, net_utils, delta, verbose=True):
        """
    ...
    def compute_distance_to_summit(self):
        """
        For each genotype, calculates its distance from the summit genotype,
        i.e., the genotype with the highest score.

        :return: (list) Each element is the length of the shortest path between
                    a genotype and the summit. Indices in the list correspond
                    to vertex Ids in the network.

        """

        # The summit genotype
        summit = Utils.getSeqWithMaxScore(
            self._network, self._bit_manip.seqLength)

        # Vertex that represents the summit genotype
        target_vertex = self._net_utils.getVertex(summit, self._network)

        return [
            len(
                self._network.get_shortest_paths(
                    source_vertex,
                    to=target_vertex,
                    weights=None,
                    mode=igraph.OUT,
            ...
            for source_vertex in self._network.vs
        ]
