...
from genonets.errors import GenonetsError
...
class Writer:
    ...
    def writeInParamsToFile(paramsDict, path):
        ...
        # Sort the dict in ascending order of alphabet
        paramsDict = dict(
        ...
        try:
            with open(fileName, "w") as outFile:
                ...
                for param in paramsDict:
                    outFile.write(f'{param}: {paramsDict[param]}\n')
        except Exception:
            print("Error: " +
    ...
    def writeNetAttribs(repToNetDict, repToGiantDict, netBuilder, path, attrsToIgnore, attribute_order,
        ...
        fileName = path + "Genotype_set_measures.tsv"
    ...
    def writeSeqAttribs(repToGiantDict, path, attrsToIgnore, order, repertoires=gc.ALL):
        ...
        for repertoire in repertoires:
            ...
            fileName = path + repertoire + "_genotype_measures.tsv"

            # If there are no attributes to write,
            attributes = repToGiantDict[repertoire].vs.attributes()
            if not (set(attributes) - set(attrsToIgnore())):
                return

            # Open file to write
            with open(fileName, 'w') as dataFile:
                Writer.writeSeqAttribsFor(
                    repToGiantDict[repertoire],
                    dataFile,
                    attrsToIgnore,
                    order
                )
    ...
    def writeOverlapToFile(overlapMat, repertoires, path):
        fileName = path + "Genotype_set_overlap.tsv"
